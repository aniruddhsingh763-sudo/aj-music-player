...
# 2. UI & JAVASCRIPT (Multi-Play Block & Pattern)
st.markdown("""
    <style>
    /* Background Pattern */
    .stApp { 
    ...
    div.stButton > button {
        ...
        color: white !important; font-weight: bold; height: 65px; width: 100%;
    }
    audio { width: 100%; filter: invert(100%) hue-rotate(85deg) brightness(1.6); }
    #MainMenu, footer, header {visibility: hidden;}
    </style>
    
    <script>
    // System-wide fix: Pause others when a new song starts
    document.addEventListener('play', function(e){
        var audios = document.getElementsByTagName('audio');
        for(var i = 0, len = audios.length; i < len; i++){
            if(audios[i] != e.target){
                audios[i].pause();
                audios[i].currentTime = 0; // Optional: Reset purana gaana
            }
        }
    }, true);
    </script>
...
if 'q' not in st.session_state: st.session_state.q = "New Haryanvi Songs 2026"

# --- SEARCH & MOODS ---
st.markdown("<h1 style='text-align: center; color: white;'>Aj BEATs Studio 🎧</h1>", unsafe_allow_html=True)
search_val = st.text_input("", placeholder="🔍 Search songs...", key="search_bar")

if search_val and search_val != st.session_state.q:
    st.session_state.q = search_val
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        try:
            with st.spinner('Syncing...'):
                info = ydl.extract_info(st.session_state.q, download=False)
                st.session_state.playlist = [e for e in info['entries'] if e is not None]
        except: st.error("Slow Internet! Refresh karke 'Fetch' dabao.")
...
with c2: 
    if st.button("🚀 FETCH NOW"): st.session_state.playlist = []; st.rerun()
...
for song in songs:
    with st.container():
        ...
        """, unsafe_allow_html=True)
        # 🟢 AUTO-LOOP Individual Toggle (AS PER YOUR IMAGE)
...
st.markdown("<br><center><small style='color:gray;'>AJ BEATS • v200.0 • No More Multi-Play</small></center>", unsafe_allow_html=True)
