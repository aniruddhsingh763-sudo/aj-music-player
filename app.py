...
# 1. Technical Fix: Saari faltu warnings ko backend se hi silent kar diya hai
logging.getLogger("streamlit.runtime.scriptrunner_utils.script_run_context").setLevel(logging.ERROR)

# 2. Page Setup
st.set_page_config(page_title="Aj Pro Player", layout="wide", initial_sidebar_state="collapsed")

# 3. Custom CSS - Spotify Style & Error Free UI
st.markdown("""
    <style>
    ...
    /* Mobile optimization */
    @media (max-width: 600px) { .profile-img { width: 80px; height: 80px; } }
    </style>
...
# --- Search (FIXED: label_visibility hatata hai warning) ---
user_search = st.text_input(label="Search", value="", placeholder="🔍 Gaana ya Singer search karein...", label_visibility="collapsed")

# --- Moods Selection (Top Buttons) ---
...
with m_col2:
    if st.button("🕺 Punjabi"): mood = "New Punjabi Hits 2026"
with m_col3:
    if st.button("📻 Old Gold"): mood = "Bollywood 90s Evergreen"
with m_col4:
    if st.button("💔 Sad"): mood = "Arijit Singh Sad Collection"
with m_col5:
    if st.button("🥳 Party"): mood = "New Bollywood Dance Songs"
with m_col6:
    if st.button("🧘 Lofi"): mood = "Hindi Lofi Chill Mix"
...
# --- Fetch & Play ---
# Technical Fix: Added 'ignoreerrors' and 'quiet' to stop logs spamming
ydl_opts = {
                ...
                st.markdown(f"""
                    <div class="song-card">
                        <div style="display: flex; align-items: center; gap: 10px;">
                            ...
                            <div>
                                <b style="font-size:14px; color:#fff;">{song.get('title')[:55]}</b><br>
...
st.markdown(f"""
    <div style="background-color: #121212; padding: 20px; border-radius: 15px; margin-top: 50px; text-align: center; border-top: 2px solid #1DB954;">
        <p style="color:#1DB954; font-weight:bold;">Aj Pro Player v4.0 Final</p>
        <p style="font-size: 11px; color: #555;">Repeat: Player ⋮ menu -> 'Loop' | Search singer for playlist</p>
    </div><br>
    """, unsafe_allow_html=True)
